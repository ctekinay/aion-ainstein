...
# Shared timeout + retry config for all OpenAI-compatible clients.
# Caps total wait at ~3 attempts x 60s = ~180s, matching agent-level timeouts.
# Without this, the OpenAI httpx client retries indefinitely (observed: 23 min).
_OPENAI_CLIENT_DEFAULTS = {
    "max_retries": 2,
...
class Settings(BaseSettings):
    ...
    def get_openai_client_kwargs(self, provider: str = None) -> dict:
        ...
        if provider == "openai":
            return {**_OPENAI_CLIENT_DEFAULTS, "api_key": self.openai_api_key}
