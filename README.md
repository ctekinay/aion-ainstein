...
- **On-demand** — domain-specific skills (ArchiMate generation, ArchiMate views, SKOSMOS vocabulary, principle
  generation, principle quality assessment) are injected only when the Persona emits matching `skill_tags` (e.g.,
  `["archimate"]`, `["vocabulary"]`, `["generate-principle"]`, or `["principle-quality"]`). A standard KB query like
  "What ADRs exist?" never loads the ArchiMate generation skill (~10K chars), SKOSMOS vocabulary rules, or principle
  instructions.
