...
class PolicyChunkingStrategy(ChunkingStrategy):
    ...
    def _detect_headers(self, content: str) -> list[tuple[int, str, int]]:
        ...
        # Numbered headers from DOCX/PDF business documents
        # Matches: "1\tInleiding", "2.1\tScope", "2.2.1\tKwaliteitsprincipes"
        # Level derived from numbering depth: 1 → level 1, 1.1 → level 2, etc.
        numbered_headers = re.finditer(
            r"^(\d+(?:\.\d+)*)\s*\t\s*(.+)$", content, re.MULTILINE
        )
    ...
    def chunk_document(
        ...
        if headers:
            ...
            # No headers detected — split into paragraph-based chunks.
            # Use SECTION (not SEMANTIC_UNIT) so these chunks are included
            # when index_section_level=True / include_granular=False.
