...
class TestSessionContext:
    ...
    def test_queue_forwarding_accepts_event_objects(self):
        """Phase 1a.3-bridge regression: chat_ui._stream_agent_response's
        queue-forwarding loop must access Event objects via attributes
        (event.type) and serialize via to_sse() — not via dict subscript
        and json.dumps. After 1a.3, the queue contains Event objects (not
        dicts), and the pre-1a access patterns would fail at runtime.

        Exercises the iteration-limit trigger — the path that actually
        runs every time an agent burns its budget — through the exact
        Event-attribute + to_sse() shape the forwarding loop now uses.
        """
        from queue import Queue
        ...
        q = Queue()
        ctx = SessionContext(event_queue=q, max_tool_calls=1, agent_label="RAG Agent")
        ctx.check_iteration_limit()  # 1 <= 1 → False, no emit
        ctx.check_iteration_limit()  # 2 > 1 → True, emits Event

        queued = q.get_nowait()
        # 1. The queue holds an Event (not a dict).
        assert isinstance(queued, Event)
        # 2. event.type works (the attribute the forwarding loop reads).
        assert queued.type == "status"
        # 3. event.to_sse() works (replaces json.dumps in the yield).
        sse = queued.to_sse()
        assert sse.startswith("data: ")
        assert sse.endswith("\n\n")
        assert "Tool call limit reached" in sse
        # 4. The wire-format remains valid SSE JSON the browser can parse.
        import json as _json
        payload = _json.loads(sse[len("data: "):].rstrip("\n"))
        assert payload["type"] == "status"
        assert "Tool call limit reached" in payload["content"]

    def test_check_iteration_limit_logs_at_warning(self, caplog):
        """Phase 1a.3.1 regression guard: tripping the limit must emit a
        WARNING-level log line (the ``log_level=logging.WARNING`` override
        in emit_event). Pre-1a this was an explicit ``logger.warning``
        call alongside the emit; post-1a the tee carries the severity.
        Easy-to-accidentally-drop parameter — pin it.
        """
        ctx = SessionContext(max_tool_calls=1, agent_label="TestAgent")
        caplog.set_level(logging.DEBUG, logger="aion.agents")
        ctx.check_iteration_limit()  # 1 <= 1 → False, no emit
        ctx.check_iteration_limit()  # 2 > 1 → True, emit at WARNING
        warning_records = [r for r in caplog.records if r.levelno == logging.WARNING]
        assert any(
            "Tool call limit reached" in r.getMessage()
            for r in warning_records
        ), [r.getMessage() for r in caplog.records]

    def test_emit_event_with_queue(self):
        """Phase 1a.2.b: emitting a typed Event puts the typed object on
        the queue (not a dict). The browser-side SSE serialisation
        happens only at the outermost StreamingResponse boundary via
        ``Event.to_sse()`` — internal consumers read attributes directly.
        """
        q = Queue()
        ctx = SessionContext(event_queue=q)
        ctx.emit_event(Event(type="status", content="test"))
        queued = q.get_nowait()
        assert isinstance(queued, Event)
        assert queued.type == "status"
        assert queued.content == "test"

    def test_emit_event_without_queue(self):
        """No queue → emit_event must not raise. Pre-1a behaviour
        preserved (the tee-to-logs path still fires regardless).
        """
        ctx = SessionContext()
        # Must not raise.
        ctx.emit_event(Event(type="status", content="test"))

    def test_emit_event_dict_shim_warns_and_converts(self, caplog):
        """Phase 1a.2.b transitional shim: emitting a legacy dict logs a
        deprecation warning AND converts the dict to a typed Event before
        queueing. The warning is the migration-tracking signal — the
        final commit of Phase 1a removes the shim only after the warning
        stops firing across the codebase.
        """
        q = Queue()
        ctx = SessionContext(event_queue=q)
        caplog.set_level(logging.WARNING, logger="aion.agents")
        ctx.emit_event({"type": "status", "content": "legacy"})
        # Queue holds the converted typed Event.
        queued = q.get_nowait()
        assert isinstance(queued, Event)
        assert queued.type == "status"
        assert queued.content == "legacy"
        # Deprecation warning fired — migration-tracking signal.
        assert any(
            "emit_event called with dict" in r.getMessage()
            for r in caplog.records
        )

    def test_emit_event_tees_to_logs_at_default_level(self, caplog):
        """Tee-to-logs substrate for Phase 1a.2.c: status defaults to INFO,
        error defaults to ERROR. ``check_iteration_limit`` overrides via
        ``log_level=logging.WARNING`` (see test below).
        """
        ctx = SessionContext()
        caplog.set_level(logging.DEBUG, logger="aion.agents")

        ctx.emit_event(Event(type="status", agent="rag", content="searching"))
        info_records = [r for r in caplog.records if r.levelno == logging.INFO]
        assert any("searching" in r.getMessage() for r in info_records)

        caplog.clear()
        ctx.emit_event(Event(type="error", agent="repo_analysis", content="boom"))
        error_records = [r for r in caplog.records if r.levelno == logging.ERROR]
        assert any("boom" in r.getMessage() for r in error_records)

    def test_emit_event_log_level_override(self, caplog):
        """``log_level=`` overrides the type-default — used by
        ``check_iteration_limit`` to log a ``type=status`` at WARNING.
        """
        ctx = SessionContext()
        caplog.set_level(logging.DEBUG, logger="aion.agents")
        ctx.emit_event(
            Event(type="status", agent="rag", content="escalated"),
            log_level=logging.WARNING,
        )
        warning_records = [r for r in caplog.records if r.levelno == logging.WARNING]
        assert any("escalated" in r.getMessage() for r in warning_records)
        # And NOT at INFO (the default for status).
        info_records = [r for r in caplog.records if r.levelno == logging.INFO]
        assert not any("escalated" in r.getMessage() for r in info_records)

    def test_emit_event_injects_agent_label(self):
        """Auto-injection (preserved from pre-1a behaviour) populates
        ``event.agent`` from ``self.agent_label`` when the event itself
        didn't set one.
        """
        q = Queue()
        ctx = SessionContext(event_queue=q, agent_label="RAG Agent")
        ctx.emit_event(Event(type="status", content="hi"))
        queued = q.get_nowait()
        assert queued.agent == "RAG Agent"

    def test_emit_event_preserves_explicit_agent(self):
        """If the emitter set ``event.agent`` explicitly, the auto-inject
        does NOT overwrite it.
        """
        q = Queue()
        ctx = SessionContext(event_queue=q, agent_label="Persona")
        ctx.emit_event(Event(type="status", agent="custom_agent", content="hi"))
        queued = q.get_nowait()
        assert queued.agent == "custom_agent"

    def test_emit_event_decision_rewrite(self):
        """Decision events still get the 'Decision: X Reasoning: Y' →
        human-readable + tool-field rewrite. The bridge in emit_event
        keeps ``_rewrite_decision`` working until Phase 1a.6 refactors
        it to attribute access.
        """
        q = Queue()
        ctx = SessionContext(event_queue=q)
        ctx.emit_event(Event(
            type="decision",
            content="Decision: search_principles Reasoning: looking up PCP.10",
        ))
        queued = q.get_nowait()
        assert queued.tool == "search_principles"
        # Reasoning becomes the user-facing content.
        assert "PCP.10" in (queued.content or "")

    def test_emit_event_decision_default_tool_when_no_decision_prefix(self):
        """A decision event whose content lacks the 'Decision:' prefix
        gets tool='unknown' (preserving pre-1a ``setdefault`` semantics
        despite the dict-bridge in emit_event).
        """
        q = Queue()
        ctx = SessionContext(event_queue=q)
        ctx.emit_event(Event(type="decision", content="just some content"))
        queued = q.get_nowait()
        assert queued.tool == "unknown"
...
class TestSkillRouting:
    ...
    def test_no_tag_collision_between_generation_and_archimate(self):
        ...
        # Post-supersession: the architecture generation skill is
        # enterpower's archimate-oxc-generator (OXC output).
        gen_entry = entries.get("archimate-oxc-generator")
        tools_entry = entries.get("archimate-tools")
        assert gen_entry is not None, "archimate-oxc-generator skill missing"
...
class TestAgentConstruction:
    ...
    def test_vocabulary_agent_has_6_tools_with_client(self):
        """Phase 1a.7: ``skosmos_define`` added as the 4th SKOSMOS tool
        (ISS-001 atomic search+define composition). With a client the
        agent has: 4 SKOSMOS tools + search_knowledge_base + request_data.
        """
        mock_client = MagicMock()
        with self._mock_model():
        ...
        tools = self._get_tool_names(agent)
        expected = {
            "skosmos_define",  # Phase 1a.7 — ISS-001 fix
            "skosmos_search",
    ...
    def test_vocabulary_agent_has_5_tools_without_client(self):
        """Without a Weaviate client, no Tier-2 ``search_knowledge_base``
        — 4 SKOSMOS tools + request_data."""
        with self._mock_model():
            from aion.agents.vocabulary_agent import _build_vocabulary_agent
            agent = _build_vocabulary_agent(None)
        tools = self._get_tool_names(agent)
        expected = {
            "skosmos_define",  # Phase 1a.7
            "skosmos_search",
            "skosmos_concept_details",
            "skosmos_list_vocabularies",
            "request_data",
