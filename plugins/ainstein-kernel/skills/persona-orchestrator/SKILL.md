...
| listing | User explicitly requests an enumeration, count, or range-based selection of documents | "List all ADRs", "What principles exist?", "How many PCPs are there?", "Show me ADRs 5 through 15", "What ADRs exist between 1 and 10?", "List principles from 10 to 20" |
...
- "Show me ADRs 5 through 15" → `listing` (range enumeration)
- "What principles exist between 10 and 20?" → `listing` (range enumeration)
...
- **No template literals or placeholders**: Never use `{n}`, `{number}`, `{id}`, `{name}` or similar format patterns in rewritten queries. Use concrete values or descriptive text. Wrong: "List ADR.{n}D approval records". Right: "List all ADR approval records (DARs)"
...
- **Do not add scope constraints**: Never add qualifiers, filters, or scope narrowing that the user did not explicitly state. "What is the total number?" means the total of whatever was just discussed — do not narrow to a subset like "ESA DARs" when the user said "total." Rewrite conservatively: preserve the user's exact scope.
...
When the query asks to **assess, evaluate, review, or judge the quality** of one or more principles, OR to **evaluate a document against the principles for compliance**, add `"principle-quality"` to skill_tags. Use this tag whenever "principles" are the evaluation criteria — whether assessing principle quality itself or checking document compliance against them.
...
- "Evaluate this document against our architectural principles" → `skill_tags: ["principle-quality"]` — compliance evaluation uses the principle agent
- "Which principles does this proposal violate?" → `skill_tags: ["principle-quality"]` — compliance check
- "Check this document for principle compliance" → `skill_tags: ["principle-quality"]`
...
**Important:** `"principle-quality"` covers two use cases: (1) assessing whether principles are well-written/quality, and (2) evaluating documents against principles for compliance. Simply mentioning, comparing, searching, or summarizing principles does NOT activate it. If the user is not asking to judge quality, fitness, or compliance, use `skill_tags: []`.
...
- **skill_tags scope:** Only assign specialist skill_tags (e.g., `generate-principle`, `archimate`) when the user is refining an artifact produced by that specialist agent. If the previous output was a comparison table, analysis, or summary produced by RAG/synthesis that happens to reference principles or ADRs, leave `skill_tags: []` so the refinement stays in the RAG/synthesis path.
...
When an ACTIVE ARTIFACT is present, the user's query likely relates to it.
Use the artifact type to inform classification:

**Uploaded documents** (document/pdf, document/docx, document/markdown):
- "Summarize this", "What does it say about X?", "List the recommendations"
  -> `retrieval` with `skill_tags: []` (document-only analysis, no KB search)
- "Check this against our principles", "Does this comply with our ADRs?"
  -> `retrieval` with `skill_tags: ["rag-quality-assurance"]` (cross-reference with KB)
- The distinction: does the user want information FROM the document, or do they want
  to COMPARE it AGAINST the knowledge base? "our", "the KB", "comply with" = cross-reference.
  "this document", "it says", "summarize" = document-only.
- When ambiguous, default to document-only (`skill_tags: []`).

**ArchiMate models** (archimate/xml, text/yaml):
- "Describe the model", "What elements are in it?" -> `inspect`
- "Add a Technology layer", "Remove the database element" -> `refinement`
- "Does this model align with our principles?" -> `retrieval` with
  `skill_tags: ["rag-quality-assurance"]` (cross-reference)

**Repository analysis** (repo-analysis/yaml):
- "Generate an ArchiMate model from this" -> `generation` with
  `skill_tags: ["archimate"]`
- "What did you find?" -> `follow_up`

When the user says "it", "this", "the model", "the document", resolve the pronoun
to the active artifact. Do not classify as off_topic or clarification when the
referent is clearly the active artifact.

## Context-Answerable Follow-ups

For a domain-specific knowledge base, `direct: true` on follow-ups should be rare. When in doubt, route to RAG — a 10-second agent call is better than a 2-second fabricated answer.

**Set `"direct": true` ONLY when ALL of these are true:**
- The question is about the agent's own reasoning, formatting choices, or conversation mechanics ("Why did you say that?", "Can you rephrase?", "What did you mean by X?")
- The answer requires ZERO domain knowledge or metadata not already displayed verbatim in the conversation
- No filtering, grouping, counting-by-property, or classification by metadata (owner_team, status, doc_type, approval date) is needed
- Examples: "Why 33 elements?", "Can you explain that last point?", "What format is that in?"

**Set `"direct": false` (route to RAG) when:**
- The question involves ANY domain knowledge, even if data "looks" available in conversation ("Which are ESA-owned?" → needs owner_team from KB)
- The question filters, groups, or classifies previous results by a property not shown in the response text
- User asks for the content of a specific document, even if that document was mentioned in conversation
- User needs new information not already in the conversation
- Examples: "Which of these are ESA DARs?" → needs RAG (owner_team filtering), "Which are accepted?" → needs RAG (status filtering), "Now compare it with PCP.13" → needs RAG, "What does ADR.29 say?" → needs RAG
...
4. **Listing intent**: Any query classified as `listing` (enumerate, count, filter, or range-based document requests).
   → `complexity: "listing"`

5. **Everything else**: direct lookups, single-document queries, topical searches, generation, follow-ups, refinements.
   → `complexity: "simple"`
