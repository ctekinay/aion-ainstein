...
def _mock_lifespan(tmp_path):
    ...
    Redirects SQLite DB to a temp path so tests don't touch the real
    chat_history.db, and calls init_db() to create the required tables.
    """
...
class TestDirectResponsePath:
    ...
    @pytest.mark.asyncio
    async def test_conversational_routes_to_rag(self, client, _mock_lifespan):
        """Phase 0d: ``conversational`` was removed from the
        direct-response allowlist. Generic architecture/engineering
        questions must route through RAG so the answer is grounded in the
        curated KB, not synthesised from the model's training data.

        Persona now emits ``direct_response=None`` plus a rewritten
        retrieval query (NOT a baked answer); the routing predicate at
        chat_ui.py:~2349 then falls through to the RAG path.
        """
        persona_result = _make_persona_result(
            intent="conversational",
            rewritten_query="trade-offs of event-driven architecture",
            direct_response=None,  # 0d invariant — never a baked answer
        )
        _mock_lifespan._persona.process = AsyncMock(return_value=persona_result)
        ...
        async def mock_rag(*args, **kwargs):
            ...
            yield Event(type="decision", agent="RAG Agent", tool="search_principles")
            yield Event(
                type="complete",
                response="Trade-offs from ADR.29 …",
                sources=[],
                timing={"total_ms": 500},
            )
        ...
        with patch("aion.chat_ui.stream_rag_response", side_effect=mock_rag):
            resp = await client.post(
                "/api/chat/stream",
                json={"message": "What are the trade-offs of event-driven architecture?"},
            )

        events = _collect_sse_events(resp.content)
        # RAG MUST be invoked — conversational no longer short-circuits.
        assert len(rag_called) == 1, "RAG was not called — conversational still on direct path"
        complete = _find_event(events, "complete")
        assert complete is not None
        # And NOT the direct path (inverse of the pre-0d assertion).
        assert complete.get("path") != "direct"
        # And agent decision events DO fire now (the pre-0d test asserted
        # ``_find_events(events, "decision") == []`` — that's exactly the
        # ungrounded-answer behaviour Phase 0d removes).
        assert _find_events(events, "decision") != []

    @pytest.mark.asyncio
    async def test_conversational_with_no_kb_grounding_declines(
        self, client, _mock_lifespan,
    ):
        """Grounding contract: when RAG has no relevant content for a
        conversational query, the system must DECLINE — never synthesise
        from training data. The decline phrasing is delivered by the
        existing RAG-prompt instructions at rag_agent.py:863 and
        chat_ui.py:~2128 ("I don't have information about that in the
        provided context"); the mock here reproduces that contract so the
        E2E layer can verify the routing carries it through unchanged.
        """
        persona_result = _make_persona_result(
            intent="conversational",
            rewritten_query="domain-driven design vs microservices for grid platform",
            direct_response=None,
        )
        _mock_lifespan._persona.process = AsyncMock(return_value=persona_result)

        decline_response = "I don't have information about that in the provided context."

        async def mock_rag_empty(*args, **kwargs):
            yield Event(type="decision", agent="RAG Agent", tool="search_adrs")
            yield Event(
                type="complete",
        ...
        with patch("aion.chat_ui.stream_rag_response", side_effect=mock_rag_empty):
            resp = await client.post(
                "/api/chat/stream",
                json={"message": "DDD vs microservices for our grid?"},
            )
        events = _collect_sse_events(resp.content)
        complete = _find_event(events, "complete")
        assert complete is not None
        assert complete.get("path") != "direct"
        # Decline phrasing — proves the grounding instruction reaches the user.
        assert "don't have information" in complete["response"].lower()
    ...
    @pytest.mark.asyncio
    async def test_follow_up_with_metadata_filter_routes_to_rag(self, client, _mock_lifespan):
        """Follow-up requiring metadata filtering must route to RAG, not direct response."""
        persona_result = _make_persona_result(
            intent="follow_up",
            rewritten_query="Which of the 59 DARs are ESA-owned?",
            direct_response=None,  # NOT direct — needs RAG for owner_team filtering
        )
        _mock_lifespan._persona.process = AsyncMock(return_value=persona_result)
        ...
        async def mock_rag(*args, **kwargs):
            rag_called.append(True)
            yield Event(type="status", agent="RAG Agent", content="Searching...")
            yield Event(type="complete", response="12 ESA-owned DARs found.", sources=[], timing={'total_ms': 500})

        with patch("aion.chat_ui.stream_rag_response", side_effect=mock_rag):
            resp = await client.post(
                "/api/chat/stream",
                json={"message": "Which of these are ESA DARs?"},
            )

        events = _collect_sse_events(resp.content)
        # RAG agent must be called — not short-circuited by direct_response
        assert len(rag_called) == 1
        complete = _find_event(events, "complete")
        assert complete is not None
        # Must NOT have path: "direct"
        assert complete.get("path") != "direct"
...
class TestRAGPath:
    ...
    async def test_retrieval_routes_to_rag_agent(self, client, _mock_lifespan):
        ...
        async def mock_rag_stream(*args, **kwargs):
            yield Event(type="status", agent="RAG Agent", content="Searching...")
            yield Event(type="complete", response="18 ADRs found.", sources=[{'title': 'ADR.01'}], timing={'total_ms': 500})
    ...
    async def test_rag_error_emits_error_event(self, client, _mock_lifespan):
        ...
        async def mock_rag_error(*args, **kwargs):
            yield Event(type="error", content="Weaviate connection failed")
...
class TestGeneralKnowledgeFallbackE2E:
    ...
    async def test_fallback_includes_disclaimer_prefix(self, client, _mock_lifespan):
        ...
        async def mock_rag_with_fallback(*args, **kwargs):
            yield Event(type="complete", response=fallback_response, sources=[], timing={'total_ms': 800})
...
class TestRepoAnalysisRouting:
    ...
    async def test_repo_analysis_routes_to_stream_repo_archimate(self, client, _mock_lifespan):
        ...
        async def mock_repo_archimate(*args, **kwargs):
            ...
            yield Event(type="status", agent="Repository Analysis", content="Cloning repository...")
            yield Event(type="status", agent="Repository Analysis", content="Profile complete: 220 files")
            yield Event(type="status", agent="ArchiMate", content="Generating model...")
            yield Event(type="complete", response="Generated ArchiMate model with 12 elements.", sources=[], timing={'total_ms': 5000})
    ...
    async def test_repo_analysis_not_triggered_for_inspect_intent(self, client, _mock_lifespan):
        ...
        REPO_ANALYSIS is excluded from the inspect override because it has
        its own dedicated gate requiring intent=generation. Stale repo-analysis
        tags from previous turns must not hijack inspect."""
        persona_result = _make_persona_result(
            intent="inspect",
        ...
        async def mock_repo(*args, **kwargs):
            ...
            yield Event(type="complete", response="repo analysis", sources=[], timing={})

        async def mock_inspect(*args, **kwargs):
            inspect_called.append(True)
            yield Event(type="complete", response="inspect result", sources=[], timing={})
...
class TestVocabularyRouting:
    ...
    async def test_vocabulary_skill_tag_routes_to_vocabulary_agent(self, client, _mock_lifespan):
        ...
        async def mock_vocab(*args, **kwargs):
            ...
            yield Event(type="status", agent="Vocabulary", content="Searching SKOSMOS...")
            yield Event(type="complete", response="Active power is the rate of energy transfer.", sources=[{'title': 'IEC 61970'}], timing={'total_ms': 200})
...
class TestGenerationRouting:
    ...
    async def test_generation_intent_routes_to_generation_pipeline(self, client, _mock_lifespan):
        ...
        async def mock_gen(*args, **kwargs):
            call_log.append(True)
            yield Event(type="status", content="Generating...")
            yield Event(type="artifact", artifact_id="abc123", filename="adr29.archimate.xml")
            yield Event(type="complete", response="Generated model with 8 elements.", sources=[], timing={'total_ms': 3000})
    ...
    async def test_generation_not_triggered_for_retrieval(self, client, _mock_lifespan):
        ...
        async def mock_gen(*args, **kwargs):
            ...
            yield Event(type="complete", response="generated", sources=[], timing={})

        async def mock_rag(*args, **kwargs):
            rag_called.append(True)
            yield Event(type="complete", response="RAG result about ArchiMate", sources=[], timing={'total_ms': 300})
...
class TestPrincipleRouting:
    ...
    async def test_principle_generation_routes_to_principle_agent(self, client, _mock_lifespan):
        ...
        async def mock_principle(*args, **kwargs):
            call_log.append(True)
            yield Event(type="complete", response="PCP.42: Data Sovereignty...", sources=[], timing={'total_ms': 1500})
