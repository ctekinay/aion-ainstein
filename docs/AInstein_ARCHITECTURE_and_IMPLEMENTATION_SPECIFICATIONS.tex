...
Modules & \texttt{src/response\_schema.py}, \texttt{src/response\_gateway.py}, \texttt{src/llm\_client.py} \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Response Gateway}

\textbf{Module:} \texttt{src/response\_gateway.py}

The response gateway is the unified API contract layer---\textbf{all UI API responses must pass through this}. It sits at the UI boundary and enforces the structured response contract.
...
\subsection{CLI Delimiter Protocol}
...
\textbf{Extraction Priority Order:}

\begin{enumerate}
    \item \textbf{Explicit markers} (highest confidence): \texttt{<<<JSON>>>...<<<END\_JSON>>>}
    \item \textbf{Raw JSON}: Entire response is valid JSON
    \item \textbf{Fenced JSON block}: \texttt{```json...```}
    \item \textbf{Embedded JSON}: Regex extraction of \texttt{\{...\}} blocks
\end{enumerate}
...
\subsection{Gateway Enforcement Policies}

\begin{table}[h]
\centering
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Policy} & \textbf{Behavior} & \textbf{Use Case} \\
\midrule
\texttt{strict} & Fail with controlled error if JSON invalid & Production UI (default) \\
\texttt{soft} & Log and degrade to raw text & Development/debugging \\
\texttt{lenient} & Allow embedded JSON extraction & Legacy compatibility \\
\bottomrule
\end{tabular}
\caption{Gateway enforcement policies}
\end{table}

\subsection{Failure UX}
...
\textbf{Example user-facing error:}
...
\subsection{Structured Mode Context}
...
\subsection{Weaviate Count Population}

The gateway overwrites LLM-guessed \texttt{items\_total} with authoritative Weaviate counts:
...
\textbf{Module:} \texttt{src/llm\_client.py}
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Client} & \textbf{Backend} & \textbf{Use Case} \\
\midrule
\texttt{ElysiaClient} & Elysia Tree & Primary production client with agentic tool selection \\
\texttt{DirectLLMClient} & Direct Weaviate + LLM & Fallback when Elysia fails \\
\texttt{ResilientLLMClient} & Wrapper & Automatic fallback between clients \\
\bottomrule
\end{tabular}
\caption{LLM client types}
\end{table}
...
\subsection{QueryResult Structure}
...
\subsection{Resilient Client with Automatic Fallback}
...
1.1 & February 2026 & Added Response Gateway and LLM Client Abstraction sections \\
\bottomrule
\end{tabular}
...
\section{Key Source Modules}

\begin{itemize}
    \item \texttt{src/response\_schema.py} --- Core schema, parser, validator, metrics, cache
    \item \texttt{src/response\_gateway.py} --- Unified API contract layer with CLI delimiter protocol
    \item \texttt{src/llm\_client.py} --- Vendor-agnostic LLM client abstraction
    \item \texttt{src/elysia\_agents.py} --- Integration with RAG system
    \item \texttt{tests/test\_structured\_response\_contract.py} --- Contract enforcement tests
\end{itemize}
