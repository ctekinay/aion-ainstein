...
def _git_metadata(repo_path: str) -> dict:
    ...
    try:
        sha = subprocess.run(
            ...
            capture_output=True, text=True, timeout=settings.timeout_subprocess_quick,
        )
        if sha.returncode == 0:
...
def git_diff_stats(repo_path: str, base_branch: str) -> dict:
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True, text=True, timeout=settings.timeout_subprocess_diff,
        )
...
def clone_repo(url_or_path: str) -> dict:
    ...
    # Authenticate the clone via -c http.extraHeader so the token never appears
    # in the URL, process listings (ps aux), or git's reflog. Private repos
    # (e.g. internal Alliander) fail unauthenticated with a "not found" message
    # — GitHub deliberately conceals existence (ISS-003b). The same pattern is
    # used in aion/mcp/github.py:94 and :153. Note: do NOT log the cmd list —
    # it contains the bearer token. We log result.stderr only (git does not
    # echo the auth header into stderr).
    ...
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=settings.timeout_subprocess_clone)
        if result.returncode != 0:
            # ISS-003a server-side logging — Phase 1a.2.c: the explicit
            # ``logger.error`` calls that Phase 0c added here have been
            # removed. The agent layer (repo_analysis_agent.clone_repo)
            # now emits a status event with ``log_level=logging.ERROR``
            # on this failure path, and ``SessionContext.emit_event``'s
            # tee logs at ERROR for us — single source of truth, no
            # double-logging. cmd is intentionally NOT logged anywhere
            # (it embeds the bearer token from Phase 0b).
            return {"error": f"Git clone failed: {result.stderr.strip()[:200]}"}
    except subprocess.TimeoutExpired:
        return {"error": f"Git clone timed out after {settings.timeout_subprocess_clone}s"}
    except FileNotFoundError:
        return {"error": "git command not found"}

    size_mb = _dir_size_mb(target)
    if size_mb > settings.max_clone_size_mb:
        import shutil
        shutil.rmtree(target, ignore_errors=True)
        return {
            "error": (
                f"Cloned repository is {size_mb:.0f} MB, exceeding the {settings.max_clone_size_mb} MB limit. "
