...
async def _rag_events(*responses: str) -> AsyncGenerator[Event, None]:
    """Yield a sequence of complete Event objects for mocking stream_rag_response.

    Phase 1a.4: yields typed ``Event`` objects (orchestrator consumes Events,
    not SSE strings).
    """
    for response in responses:
        yield Event(type="complete", response=response, sources=[], timing={})


async def _rag_status_then_complete(response: str) -> AsyncGenerator[Event, None]:
    """Simulate realistic RAG output: status + decision events followed by complete."""
    yield Event(type="status", content="Searching...")
    yield Event(type="decision", content="Calling search_principles")
    yield Event(type="complete", response=response, sources=[], timing={})


def _collect_events(raw_events: list[Event]) -> list[Event]:
    """Pass-through helper. Phase 1a.4: ``MultiStepOrchestrator.run`` yields
    typed Event objects directly — no SSE string parsing needed. The helper
    is kept so the ``parsed = _collect_events(events)`` line stays stable
...
class TestMultiStepExecution:
    ...
    async def test_two_steps_yield_status_per_step(self):
        ...
        async def mock_rag(query, **kwargs):
            yield Event(type="complete", response=f"Result for {query}", sources=[], timing={})
    ...
    async def test_synthesis_status_event_emitted(self):
        ...
        async def mock_rag(query, **kwargs):
            yield Event(type="complete", response="some result", sources=[], timing={})
    ...
    async def test_complete_event_contains_synthesis_text(self):
        ...
        async def mock_rag(query, **kwargs):
            yield Event(type="complete", response="kb result", sources=[], timing={})
    ...
    async def test_text_tokens_yielded_during_synthesis(self):
        ...
        async def mock_rag(query, **kwargs):
            yield Event(type="complete", response="result", sources=[], timing={})
...
class TestAllStepsEmpty:
    ...
    async def test_empty_rag_responses_yield_fallback_complete(self):
        ...
        async def mock_rag(query, **kwargs):
            yield Event(type="complete", response="", sources=[], timing={})
...
class TestStepResultLabeling:
    ...
    async def test_step_labels_in_synthesis_input(self):
        ...
        async def mock_rag(query, **kwargs):
            ...
            call_index += 1
            yield Event(type="complete", response=resp, sources=[], timing={})
...
class TestInnerEventSuppression:
    ...
    async def test_inner_decision_events_not_forwarded(self):
        ...
        async def mock_rag(query, **kwargs):
            yield Event(type="decision", content="Calling search_principles")
            yield Event(type="status", content="Inner status")
            yield Event(type="complete", response="result", sources=[], timing={})
        ...
        inner_status = [e for e in parsed if e.type == "status" and e.content == "Inner status"]
        assert inner_status == []
    ...
    async def test_orchestrator_status_events_are_forwarded(self):
        ...
        async def mock_rag(query, **kwargs):
            yield Event(type="complete", response="r", sources=[], timing={})
