...
class ADRChunkingStrategy(ChunkingStrategy):
    ...
    SECTION_PATTERNS = [
        ...
        (r"^##?\s*Decision\s+Drivers?\s*$", "drivers", "Decision Drivers"),
        (r"^##?\s*Decision(?:\s+Outcome)?\s*$", "decision", "Decision"),
        (r"^##?\s*Consequences\s*$", "consequences", "Consequences"),
        (r"^##?\s*More\s+Information\s*$", "consequences", "Consequences / Implications"),
        (r"^##?\s*(?:Architecture\s+)?Implications?\s*$", "consequences", "Consequences / Implications"),
        (r"^##?\s*Considered\s+Options?\s*$", "options", "Considered Options"),
        (r"^##?\s*Pros?\s+and\s+Cons?\b.*$", "proscons", "Pros and Cons"),
