...
## Section 0.1: Definitions vs Structure

This skill describes the **structure, naming, numbering, and relationships** of ESA documents. It does NOT provide formal definitions of terms.

**For formal definitions** of what an ADR, PCP, DAR, Principle, or any other term means, defer to `skosmos_search`. SKOSMOS holds the company-wide accepted, formally approved definitions. Do not use the descriptions in this skill as definitions — they are structural context, not canonical definitions.

Example:
- "What is an ADR?" → Use `skosmos_search` (definition question)
- "What is a DAR?" → Use `skosmos_search` (definition question)
- "What is a Principle?" → Use `skosmos_search` (definition question)
- "Where are ADRs stored?" → Use this skill (structural question)
- "What sections does an ADR have?" → Use this skill (structural question)
- "What ADRs exist?" → Use Weaviate `list_adrs` (listing question)
- "Show all approval records" → Use `list_dars` (DAR listing question)
- "Who approved ADR.12?" → Use this skill + Weaviate (approval lookup)

When a user asks "What is [term]?", always call `skosmos_search` first to get the formal definition. Then use this skill to add structural context if relevant (e.g., "An ADR captures architecture decisions..."). This structural context is supplementary — it always follows a SKOSMOS hit, it is NOT the Tier 2 fallback defined in the skosmos-vocabulary skill.

## Document Identifier Formatting

When displaying ADR or PCP identifiers in responses, use the canonical 2-digit format from the ESA registry: `ADR.00`, `ADR.12`, `PCP.10`, `PCP.39`. Do NOT use the 4-digit internal format (`ADR.0000`, `PCP.0010`) — that is for Weaviate filtering only. For Decision Approval Records (DARs), append D: `ADR.00D`, `PCP.10D`.

## Section 1: Document Types

There are four types of content documents:
...
Note: Some principles (PCP.21-PCP.38, PCP.41-PCP.50) are written in Dutch, reflecting their ownership by the Business Architecture Group, Data Office, Netbeheer Nederland, and Enterprise Architecture Group.

### Policy Documents

**Policy documents** are formal data governance policies published as DOCX/PDF files. They are NOT markdown, NOT numbered like ADRs or PCPs, and are stored in a separate PolicyDocument collection.

- **DO policies** (Data Office, 8 docs): Data classification, information governance, data quality, metadata management, data integration, data product classification
- **Corporate policies** (Corporate Governance, 2 docs): Privacy policy, strategic security policy
- **Language**: Primarily Dutch
- **No ID scheme**: Policies are identified by title and file_path, not by numbered identifiers

**Key distinction**: Policies are governance documents that PRESCRIBE rules and compliance requirements. Principles (PCPs) are architectural guidance that GUIDES decisions. When a user says "policy" or "beleid", they mean these DOCX/PDF documents, not PCPs — even though PCP.31-38 (Data Office principles) cover similar data governance topics.
...
A **Document Registry Index** is attached to this skill as a reference file. It lists all 59 ADRs and PCPs with their ID, Title, Status, Date, and Owner group (ESA, BA, DO, NB-EA, or EA — see Section 7).
...
| NB-EA | Netbeheer Nederland Enterprise Architecture Group | PCP.41-48 | Acceptance |
| EA | Alliander Enterprise Architecture Group | PCP.49-50 | Acceptance |
