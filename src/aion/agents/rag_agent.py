...
# 4 pairs = 8 messages — enough for multi-turn reasoning without blowing tokens.
...
def _process_history(
    ...
    Keeps the last _MAX_HISTORY_PAIRS turn pairs. If a tool call or tool
    return sits at the truncation boundary, includes the extra message(s)
    to avoid corrupted tool interaction sequences.
    """
    result: list[ModelMessage] = []

    # Prepend running summary as synthetic context if available
    ...
    # Cap to last N messages, preserving pair integrity
    max_msgs = _MAX_HISTORY_PAIRS * 2
    if len(messages) <= max_msgs:
        result.extend(messages)
        return result

    # Walk backwards from the cut point to find a clean boundary.
    # A clean boundary is a ModelRequest that isn't a tool return
    # (i.e., it starts a new user turn, not a mid-tool-interaction).
    start = len(messages) - max_msgs
    while start > 0 and isinstance(messages[start], ModelResponse):
        start -= 1
    if start < 0:
        start = 0
...
def _build_rag_agent(toolkit: RAGToolkit) -> Agent[SessionContext, str]:
    ...
    agent: Agent[SessionContext, str] = Agent(
        ...
        retries=1,
        history_processors=[_process_history],
    )
    ...
    def _build_system_prompt(
        ...
        if artifact_context:
            ...
            parts.extend([
                ...
                "_ Add a particular focus (not limited to) on identifying any gaps in the document compared to the KB, especially ESA Architecture Principles,",
