...
Usage:
    python -m src.aion.evaluation.test_runner
    python -m src.aion.evaluation.test_runner --provider ollama
    python -m src.aion.evaluation.test_runner --provider openai
    python -m src.aion.evaluation.test_runner --quick  # Run only 10 questions
"""
...
async def run_tests(provider: str = "ollama", model: str = None, quick: bool = False, debug: bool = False, verbose: bool = False, skip_health_check: bool = False) -> dict:
    ...
    if not skip_health_check:
        ...
        if provider == "ollama" and not health["ollama"]:
            ...
            print("   2. Use OpenAI: python -m src.aion.evaluation.test_runner --openai")
            print()
