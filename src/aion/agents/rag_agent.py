...
def _build_rag_agent(toolkit: RAGToolkit, mcp_tools=None) -> Agent[SessionContext, str]:
    """Build the Pydantic AI agent with 9 static tools + any plugin MCP tools.

    Static tools: 8 RAG search/list + request_data. Optional ``mcp_tools``
    are plugin-supplied callables (synthesized by aion.mcp.tool_bridge)
    that the per-skill routing rule (D10) determined should attach to
    this agent type.
    """
...
class RAGAgent:
    ...
    def __init__(self, client: WeaviateClient, mcp_tools=None):
        from aion.agents.quality_gate import ResponseQualityGate

        self.client = client
        self.toolkit = RAGToolkit(client)
        self._agent = _build_rag_agent(self.toolkit, mcp_tools=mcp_tools)
    ...
    async def _direct_query(
        ...
        This is a degraded-mode path — see the project's development guide
