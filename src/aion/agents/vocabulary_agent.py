...
from aion.tools.skosmos import (
    skosmos_define as _skosmos_define,
)
from aion.tools.skosmos import (
...
def _build_vocabulary_agent(toolkit: RAGToolkit | None, mcp_tools=None) -> Agent[SessionContext, str]:
    """Build the Pydantic AI agent with vocabulary tools + any plugin MCP tools."""
    ...
    def skosmos_define(
        ...
        """Define a term using SKOSMOS — atomic search + definition lookup
        for the single-vocab case. Use this for "what is X?" / "define X"
        queries instead of chaining skosmos_search + skosmos_concept_details
        manually.

        Returns the authoritative definition in ONE call when the term
        resolves to a single vocabulary. If multiple vocabularies have the
        term (e.g. "asset" in ESAV / IEC 61968 / IEC 62443), returns a
        disambiguation list — present the options to the user before
        drilling further.

        Args:
            term: The term or phrase to define.
            lang: Language code (default "en").
            vocab: Optional vocabulary ID to scope the lookup.

        Returns:
            Single-vocab:    {"definition", "vocabulary", "uri", "prefLabel", "hit_count"}
            Multi-vocab:     {"disambiguation": [hits...], "vocabularies": [str...]}
            No match:        {"error": "no matches", "term": term}
        """
        if ctx_.deps.check_iteration_limit():
            return {"error": "Tool call limit reached"}
        elapsed = elapsed_ms(ctx_.deps._query_start)
        ctx_.deps.emit_event(Event(
            type="decision",
            content=f"Decision: skosmos_define Reasoning: Defining '{term}' atomically",
            elapsed_ms=elapsed,
        ))
        result = _skosmos_define(term=term, lang=lang, vocab=vocab)

        # If single-vocab resolution succeeded, the LLM doesn't need to
        # call concept_details after. If multi-vocab, populate the
        # existing pending_disambiguations bookkeeping so the rest of
        # the agent flow handles disambiguation uniformly.
        if "disambiguation" in result:
            ctx_.deps.pending_disambiguations[term] = result["vocabularies"]
...
class VocabularyAgent:
    ...
    def __init__(self, client: WeaviateClient | None = None, mcp_tools=None):
        self.toolkit = RAGToolkit(client) if client else None
        self._agent = _build_vocabulary_agent(self.toolkit, mcp_tools=mcp_tools)
    ...
    async def query(
        ...
        ctx = SessionContext(
            ...
            max_tool_calls=_get_max_tool_calls("vocabulary_agent", 8),
            running_summary=running_summary,
        )

        logger.info(f"VocabularyAgent processing: {question}")
        logger.info("vocabulary_agent_model model=%s", self._agent.model.model_name)
