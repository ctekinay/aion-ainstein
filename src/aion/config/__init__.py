...
class Settings(BaseSettings):
    ...
    # Project root — found by walking up to the pyproject.toml marker.
    # Do NOT count .parent calls: this module's nesting depth has changed
    # before (config.py → config/__init__.py in e088468), which silently
    # relocated chat_history.db into src/. Anchor on a stable marker instead.
    @property
    def project_root(self) -> Path:
        for parent in Path(__file__).resolve().parents:
            if (parent / "pyproject.toml").is_file():
                return parent
        # Fallback: original layout depth (src/aion/<pkg>/__init__.py → repo root)
