# ArchiMate Open Exchange XML Template (Schema v3.1)

This template is derived from the official Open Group ArchiMate Open Exchange format.
The exchange format namespace (`3.0/`) is stable and does not change with ArchiMate language spec versions (3.1, 3.2). The most recent published XSD schema files are versioned `3.1/`; no `3.2/` XSD exists at opengroup.org.
...
<!--
  ...
  - Primary namespace: http://www.opengroup.org/xsd/archimate/3.0/
    (This URI is frozen — it does NOT change with ArchiMate spec version updates)
  - XSD location points to 3.1/archimate3_Diagram.xsd (most recent published schema)
...
<model
  ...
  <!-- MODEL NAME: required. xml:lang is required. Closing tag is </name> NOT </name> -->
...
### The `<name>` element
Standard XML: `</name>` is the correct closing tag for `<name>`. Always use matching tags:
```xml
<!-- CORRECT -->
