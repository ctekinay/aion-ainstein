...
consequence, not a silent break): get_skill_registry() now resolves to
the post-supersession MultiPluginRegistry (ainstein-kernel +
esa-workflow + enterpower-architecture). The legacy ainstein-core
`archimate` group (2 skills: archimate-generator/-view-generator,
shared_references=`archimate`) is superseded by enterpower's
authoritative `archimate` group (4 skills: archimate-oxc-generator,
-oxc-view-generator, -visual-composer, -viewer; OXC output;
shared_references=`archimate-shared`). The group/merge invariants are
unchanged in kind, re-pointed at the new aggregate.
"""
...
class TestSkillGroups:
    ...
    def test_groups_loaded(self):
        ...
        # Post-supersession: enterpower's archimate group has 4 members
        # (oxc-generator, oxc-view-generator, visual-composer, viewer).
        assert len(archimate.skills) == 4

    def test_group_members_have_correct_fields(self):
        r = get_skill_registry()
        entries = {e.name: e for e in r.list_skills()}

        gen = entries["archimate-oxc-generator"]
        assert gen.group == "archimate"
        assert gen.type == "skill"
        assert gen.load_order == 1

        view = entries["archimate-oxc-view-generator"]
    ...
    def test_shared_references_group_field(self):
        ...
        Post-supersession this is enterpower's `archimate` group, whose
        shared-references subdir is ``archimate-shared`` (enterpower's own
        plugin-scoped layout; A0 — plugin-scoped).
        """
        r = get_skill_registry()
        groups = r.list_groups()
        archimate = next(g for g in groups if g.name == "archimate")
    ...
    def test_no_duplicate_references(self):
        """Shared refs must NOT be duplicated into member skill dirs — the
        canonical copies live only at enterpower's
        shared-references/archimate-shared/ and are group-merged.

        Re-pointed at the real enterpower plugin paths: the legacy
        ``skills/archimate-generator/...`` relative paths no longer exist,
        so the old assertions had gone vacuously green (assert-absence on
        a path that is always absent). The positive control below proves
        the canonical shared copy genuinely exists, so the absence
        assertions are meaningful.
        """
        plugin = Path(__file__).parent.parent / "plugins" / "enterpower-architecture"
        gen_refs = plugin / "skills" / "archimate-oxc-generator" / "references"
        view_refs = plugin / "skills" / "archimate-oxc-view-generator" / "references"
        shared = plugin / "shared-references" / "archimate-shared"
        # Positive control: the canonical shared copies DO exist (so the
        # not-duplicated assertions below can actually fail).
        assert (shared / "archim-3.2-element-types.md").is_file()
        assert (shared / "archim-3.2-allowed-relations.md").is_file()
        # Not duplicated into the member skills' own references dirs.
        assert not (gen_refs / "archim-3.2-element-types.md").exists()
        assert not (gen_refs / "archim-3.2-allowed-relations.md").exists()
        assert not (view_refs / "archim-3.2-element-types.md").exists()
        assert not (view_refs / "archim-3.2-allowed-relations.md").exists()
    ...
    def test_generation_skill_lookup(self):
        """get_generation_skill returns the archimate generation skill with
        its validation_tool. Post-supersession that is enterpower's
        archimate-oxc-generator (OXC output); validation_tool unchanged.
        """
        r = get_skill_registry()
        gen = r.get_generation_skill(["archimate"])
        assert gen is not None
        assert gen.name == "archimate-oxc-generator"
