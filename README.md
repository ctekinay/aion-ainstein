...
- **Principle quality assessment queries** ("Assess a principle against TOGAF criteria") go to the **RAG Agent** with the
  **principle-quality-assessor** skill injected into the atlas. The agent retrieves the requested principles and applies
  the TOGAF Decision Gate and five quality dimensions to each.
...
├── thresholds.yaml                  # System-wide config: retrieval limits, LLM token limits,
│                                    #   agent max_tool_calls, quality gate, truncation
...
**Thresholds:** Skills can define `references/thresholds.yaml` for configurable parameters. All values are read at call
time (not startup), so changes take effect without restarting the server:

- `skills/thresholds.yaml` (system-wide):
  - `abstention.distance_threshold` (0.6) — maximum vector distance before abstaining
  - `retrieval_limits` — max documents per collection (`adr: 8`, `principle: 6`, `policy: 4`, `vocabulary: 4`)
  - `truncation` — content length limits (`content_max_chars: 800`, `max_context_results: 50`,
    `source_display_limit: 35`, etc.)
  - `llm_token_limits` — max tokens per LLM call site (`persona_reasoning: 2048`, `persona_standard: 500`,
    `summarize_reasoning: 512`, `rag_reasoning: 1000`, `chat_review_reasoning: 4096`, etc.). Reasoning model
    limits must be higher because GPT-5 spends reasoning tokens within `max_completion_tokens`
  - `agents.max_tool_calls` — max tool iterations per agent (`default: 15`, `rag_agent: 15`, `repo_analysis_agent: 12`). Keys
    match `AGENT_LABELS` in `agents/__init__.py`
  - `quality_gate` — response quality evaluation (`evaluation_max_tokens: 10`, `condensation_max_tokens: 1024`,
    proportionality prompts, abstention cleanup signals)
- `persona-orchestrator/references/thresholds.yaml`:
  - `persona.verbatim_window` (20) — how many recent messages the Persona sees verbatim
  - `persona.message_truncation_chars` (8000) — max chars per user message in history
...
**Timeout and retry settings** — all configurable via environment variables:

| Variable                     | Default | Description                                                     |
|------------------------------|--------:|-----------------------------------------------------------------|
| `TIMEOUT_OPENAI_DEFAULT`     |    60.0 | Default read timeout (seconds) for OpenAI SDK clients           |
| `TIMEOUT_GENERATION`         |   600.0 | Read timeout for ArchiMate/content generation calls             |
| `TIMEOUT_LLM_CALL`          |    30.0 | Timeout for brief LLM calls (summarize, title generation)       |
| `TIMEOUT_LLM_INSPECT`       |   120.0 | Timeout for inspection/classification calls (Persona, QA gate)  |
| `TIMEOUT_LONG_RUNNING`       |   300.0 | Timeout for RAG agent and embedding calls                       |
| `TIMEOUT_AGENT_MULTI_TOOL`   |   180.0 | Timeout for multi-tool agent runs (Pydantic AI)                 |
| `TIMEOUT_SUBPROCESS_QUICK`   |     5.0 | Timeout for quick git subprocess calls (rev-parse, symbolic-ref)|
| `TIMEOUT_SUBPROCESS_DIFF`    |    30.0 | Timeout for git diff-stats                                      |
| `TIMEOUT_SUBPROCESS_CLONE`   |   120.0 | Timeout for git clone operations                                |
| `TIMEOUT_SKOSMOS`            |    10.0 | Timeout for SKOSMOS REST API calls                              |
| `TIMEOUT_GITHUB_API`         |    15.0 | Timeout for GitHub API calls                                    |
| `OPENAI_MAX_RETRIES`         |       2 | Max HTTP retries for OpenAI SDK clients (2 retries = 3 attempts)|
| `EMBEDDING_MAX_RETRIES`      |       3 | Application-level retries for failed embedding batches          |
| `EMBEDDING_RETRY_DELAY`      |     5.0 | Seconds between embedding batch retries                         |
| `SUMMARIZE_TRIGGER_COUNT`    |       4 | Messages beyond verbatim window before triggering summary       |
| `MAX_CLONE_SIZE_MB`          |    1024 | Max repository size (MB) for clone operations                   |
| `MAX_README_CHARS`           |   50000 | Max README characters for GitHub repo inspection                |
| `MIN_README_CHARS_PER_REF`   |   10000 | Floor for per-repo README size when splitting across multiple repos |
