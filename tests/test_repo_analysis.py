...
class TestCloneRepo:
    ...
    def test_https_clone_failure(self, caplog):
        """Phase 1a.2.c: the explicit ``logger.error`` in clone_repo that
        Phase 0c added has been removed. clone_repo at this lower level
        returns an error dict; server-side ERROR logging now happens at
        the agent layer via the emit_event tee (see test_clone_failure_
        emits_exactly_one_error_log below). This test confirms the removal
        — clone_repo NO LONGER produces an ERROR log line on its own.
        """
        import logging
        caplog.set_level(logging.ERROR, logger="aion.tools.repo_analysis")
        with patch("aion.tools.repo_analysis.subprocess.run") as mock_run, \
             patch("aion.tools.repo_analysis.os.path.isdir", return_value=False), \
             patch("aion.tools.repo_analysis.os.makedirs"):
            ...
            # Phase 1a.2.c removal verification — zero ERROR log lines
            # from this layer. The agent emit tee is the new source of truth.
            error_records = [
                r for r in caplog.records if r.levelno == logging.ERROR
            ]
            assert error_records == [], [r.getMessage() for r in error_records]

    def test_https_clone_timeout_no_lower_level_log(self, caplog):
        """Phase 1a.2.c: timeout path's explicit ``logger.error`` removed.
        clone_repo returns an error dict without logging at ERROR.
        """
        import logging
        ...
        with patch("aion.tools.repo_analysis.subprocess.run") as mock_run, \
             patch("aion.tools.repo_analysis.os.path.isdir", return_value=False), \
             patch("aion.tools.repo_analysis.os.makedirs"):
            mock_run.side_effect = _sub.TimeoutExpired(cmd=["git"], timeout=60)
            result = clone_repo("https://github.com/alice/myrepo.git")
            assert "timed out" in result["error"].lower()
            assert [r for r in caplog.records if r.levelno == logging.ERROR] == []

    def test_https_clone_missing_git_no_lower_level_log(self, caplog):
        """Phase 1a.2.c: FileNotFoundError path's explicit ``logger.error``
        removed. clone_repo returns an error dict without logging at ERROR.
        """
        import logging
        caplog.set_level(logging.ERROR, logger="aion.tools.repo_analysis")
        with patch("aion.tools.repo_analysis.subprocess.run") as mock_run, \
             patch("aion.tools.repo_analysis.os.path.isdir", return_value=False), \
             patch("aion.tools.repo_analysis.os.makedirs"):
            mock_run.side_effect = FileNotFoundError("[Errno 2] No such file: 'git'")
            result = clone_repo("https://github.com/alice/myrepo.git")
            assert "not found" in result["error"].lower()
            assert [r for r in caplog.records if r.levelno == logging.ERROR] == []

    def test_clone_failure_emit_produces_exactly_one_error_log(self, caplog):
        """Phase 1a.2.c MANDATORY test (per the guide): emitting the
        clone-failure status event from the agent layer must produce
        EXACTLY ONE ERROR log line in caplog.

          * Two means Phase 0c's explicit ``logger.error`` wasn't fully
            removed (double-logging — bug).
          * Zero means the tee at ``SessionContext.emit_event`` isn't
            firing (regression — bug).

        Per the 1a.3.5 deviation, the agent's emit is
        ``Event(type="status", log_level=logging.ERROR)`` rather than
        ``Event(type="error", ...)`` — preserves UI semantics
        (chat_ui._capture_event treats type="error" as terminal, idling
        every agent's pixel-registry avatar) while still delivering the
        ERROR-level tee 1a.2.c needs. This test exercises that exact
        emit shape.
        """
        ...
        ctx = SessionContext(agent_label="Repository Analysis")
        ctx.emit_event(
            Event(
                type="status",
                content="Clone failed: Git clone failed: fatal: repository '...' not found",
            ),
            log_level=logging.ERROR,
        )
        error_records = [r for r in caplog.records if r.levelno == logging.ERROR]
        assert len(error_records) == 1, (
            f"expected exactly 1 ERROR log line; got {len(error_records)}: "
            f"{[r.getMessage() for r in error_records]}"
        )
        # And the content carries the failure detail.
        assert "Clone failed" in error_records[0].getMessage()


class TestCloneRepoAuth:
    """ISS-003b regression: private-repo clones must authenticate via
    ``-c http.extraHeader`` when ``GITHUB_TOKEN`` is set. The token must
    never appear in URLs/process listings, must never be logged, and the
    ``-c`` flag must come BEFORE ``clone`` (otherwise git rejects it).

    Public clones (no token in env) must continue to work unchanged.
    """

    _DUMMY_TOKEN = "ghs_FAKETOKEN1234567890abcdefghijklmnop"

    def _invoke(self, monkeypatch, token: str | None):
        """Run clone_repo against a stub subprocess and return mock_run.

        ``token`` of ``None`` clears the env var (public-clone path).
        """
        if token is None:
        ...
        with patch("aion.tools.repo_analysis.subprocess.run") as mock_run, \
             ...
             patch("aion.tools.repo_analysis._dir_size_mb", return_value=5.0):
            mock_run.return_value = MagicMock(returncode=0)
            result = clone_repo("https://github.com/alliander/internal-repo.git")
        assert "error" not in result, result
        mock_run.assert_called_once()
        return mock_run.call_args[0][0]  # argv

    def test_no_token_means_no_auth_header(self, monkeypatch):
        """Public-repo path: argv must be the pre-fix shape, no -c flag."""
        argv = self._invoke(monkeypatch, token=None)
        # Reaching `clone` immediately after `git` keeps the public flow intact.
        assert argv[:4] == ["git", "clone", "--depth", "1"], (
            f"unexpected argv shape for public clone: {argv[:6]}"
        )
        assert not any("http.extraHeader" in a for a in argv), (
            "auth header leaked into unauthenticated clone — argv: " + str(argv)
        )

    def test_empty_token_treated_as_unset(self, monkeypatch):
        """Defensive: an empty/whitespace token must NOT inject a header
        with an empty bearer value (which git would treat as malformed).
        """
        argv = self._invoke(monkeypatch, token="   \n")
        assert argv[:4] == ["git", "clone", "--depth", "1"]
        assert not any("http.extraHeader" in a for a in argv)

    def test_token_injected_before_clone(self, monkeypatch):
        """Private-repo path: ``-c http.extraHeader=AUTHORIZATION: bearer <tok>``
        must appear BEFORE the ``clone`` verb (git rejects ``-c`` after the
        subcommand).
        """
        argv = self._invoke(monkeypatch, token=self._DUMMY_TOKEN)

        # Shape:  git -c http.extraHeader=... clone --depth 1 ...
        assert argv[0] == "git"
        assert argv[1] == "-c"
        assert argv[2] == f"http.extraHeader=AUTHORIZATION: bearer {self._DUMMY_TOKEN}"
        # `clone` MUST follow the -c pair, not precede it.
        clone_idx = argv.index("clone")
        header_idx = argv.index(argv[2])
        assert header_idx < clone_idx, (
            "git -c flag must precede the `clone` verb — got: " + str(argv)
        )
        assert "--depth" in argv

    def test_token_not_emitted_to_logs(self, monkeypatch, caplog):
        """The token must not appear in any captured log record. The cmd
        list is intentionally never logged (the embedded bearer token would
        otherwise leak into the server log).
        """
        import logging
        caplog.set_level(logging.DEBUG, logger="aion.tools.repo_analysis")
        self._invoke(monkeypatch, token=self._DUMMY_TOKEN)
        for record in caplog.records:
            assert self._DUMMY_TOKEN not in record.getMessage(), (
                f"token leaked into log record: {record.name} | {record.getMessage()!r}"
            )
...
class TestExtractCodeStructure:
    ...
    def test_collaborators_scoped_to_class_body(self, tmp_path):
        """Collaborator detection must use class-body references, not file-level imports.

        Regression: previously every class in a file inherited all file-level imports
        as collaborators regardless of whether the class body referenced them.
        chat_ui.py has ~30 top-level imports, causing every class to list all 30
        as collaborators. Fix: scope to ast.walk() Name references within the class node.
        """
        pkg = tmp_path / "myapp"
        pkg.mkdir()
        (pkg / "__init__.py").write_text("")
        (pkg / "services.py").write_text("class ServiceA:\n    pass\n")
        (pkg / "handlers.py").write_text(
            "from myapp.services import ServiceA\n\n"
            "class UsesService:\n"
            "    def run(self):\n"
            "        return ServiceA()\n\n"
            "class PureHandler:\n"
        ...
        profile = {
            "architecturally_relevant_files": [
                {"path": "myapp/services.py", "tier": "T1", "category": "source_code"},
                {"path": "myapp/handlers.py", "tier": "T1", "category": "source_code"},
            ]
        }
        ...
        for mod in result["modules"]:
            for f in mod.get("files", []):
                if "handlers" in f.get("path", ""):
                    for cls in f.get("classes", []):
                        handlers_classes[cls["name"]] = cls.get("collaborators", [])

        assert "ServiceA" in handlers_classes.get("UsesService", []), \
            "UsesService references ServiceA in its body — must be a collaborator"
        assert "ServiceA" not in handlers_classes.get("PureHandler", []), \
            "PureHandler never references ServiceA — must not be listed as collaborator"
