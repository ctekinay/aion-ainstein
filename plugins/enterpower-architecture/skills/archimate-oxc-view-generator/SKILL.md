...
description: "Generates views (diagrams) for existing ArchiMate 3.2 OXC (Open Exchange) models. Use this skill when the user wants to add a new view or diagram to an ArchiMate model, visualize a specific viewpoint, or create a focused diagram from a subset of model elements."
---

**Usage**

- `/archimate-oxc-view-generator` — add a new view or diagram to an existing ArchiMate 3.2 Open Exchange XML model

# ArchiMate 3.2 View Generator

## When to use this skill

Use `archimate-oxc-view-generator` when the user wants to **add a diagram to the XML model**:
- "add a view", "create a new diagram", "add a viewpoint", "add an application view"
- "create an infrastructure diagram in the model", "add a motivation view"
- Modifying the model file structure — this skill produces XML output (a view fragment or updated model)

**Do NOT use for these — use a different skill instead:**
- "show me the model", "visualize", "open in browser" → use `archimate-visual-composer` (browser display, no XML change)
- "generate a model from this document" → use `archimate-oxc-generator` (creates elements + relationships)
