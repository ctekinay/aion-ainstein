...
from aion.persona import DIRECT_RESPONSE_INTENTS, Persona, PersonaResult, PlanStep
...
# ── Phase 0d: conversational intent invariants ───────────────────────────────

class TestConversationalNotDirect:
    """The ``conversational`` intent was removed from the direct-response
    allowlist in Phase 0d. These tests pin the invariant in two places:
    the allowlist constant, and the structural guard in
    ``PersonaResult.__post_init__`` that backs it.
    """

    def test_conversational_not_in_direct_response_intents(self):
        """Phase 0d: ``conversational`` MUST NOT be in the allowlist."""
        assert "conversational" not in DIRECT_RESPONSE_INTENTS

    def test_other_direct_intents_preserved(self):
        """Regression guard: do not accidentally widen the change."""
        assert DIRECT_RESPONSE_INTENTS == frozenset({
            "identity", "off_topic", "clarification",
        })


class TestPersonaResultPostInitGuard:
    """``PersonaResult.__post_init__`` enforces the invariant structurally
    so all four construction sites in persona.py — and any future site —
    are covered automatically.

    Each construction-time guard test pairs with a "no-op for other intents"
    control, so a permanently-firing guard would fail one of the tests.
    """

    def test_conversational_with_direct_response_is_nulled(self):
        r = PersonaResult(
            intent="conversational",
            rewritten_query=None,
            direct_response="Microservices vs monolith depends on team size...",
            original_message="What about microservices vs monolith?",
        )
        assert r.direct_response is None, (
            "structural guard did not null direct_response for conversational"
        )
        # And the user's actual question is still accessible downstream via
        # the `rewritten_query or original_message` fallback in chat_ui.py.
        assert r.original_message == "What about microservices vs monolith?"

    def test_conversational_with_no_direct_response_is_left_alone(self):
        """If the prompt edits succeed and the LLM emits direct=false +
        rewritten query, the guard must NOT touch the rewritten_query.
        """
        r = PersonaResult(
            intent="conversational",
            rewritten_query="trade-offs of event-driven architecture",
            direct_response=None,
            original_message="What are the trade-offs of EDA?",
        )
        assert r.direct_response is None
        assert r.rewritten_query == "trade-offs of event-driven architecture"

    def test_identity_with_direct_response_is_preserved(self):
        """Negative control — without this, a permanently-firing guard
        (e.g. ``self.direct_response = None`` unconditionally) would pass
        the conversational test above but break every direct intent.
        """
        r = PersonaResult(
            intent="identity",
            rewritten_query=None,
            direct_response="I'm AInstein, the ESA AI Assistant.",
            original_message="Who are you?",
        )
        assert r.direct_response == "I'm AInstein, the ESA AI Assistant."

    @pytest.mark.parametrize("intent", ["off_topic", "clarification", "retrieval", "listing"])
    def test_other_intents_with_direct_response_are_preserved(self, intent):
        """The guard fires only for conversational — every other intent
        keeps its direct_response intact.
        """
        r = PersonaResult(
            intent=intent,
            rewritten_query=None,
        ...
        assert r.direct_response == "some content"
