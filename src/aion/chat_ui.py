...
def _get_available_collections() -> list[str]:
    ...
    kb_collections = get_thresholds_value("get_kb_collections", [
...
async def stream_rag_response(question: str,
                                 ...
                                 step_index: int | None = None) -> AsyncGenerator[str, None]:
...
async def stream_document_analysis_response(
    ...
    """Stream DocumentAnalysisAgent's response as SSE text events.
    ...
    _cfg = _gtv("get_document_agent_config", {})
    _batch = _cfg.get("stream_batch_chars", 200)

    yield f"data: {json.dumps({'type': 'status', 'content': 'Analyzing uploaded document...', 'agent': AGENT_LABELS.get('document_agent', 'Document Analysis')})}\n\n"
    ...
    try:
        ...
        if partial:
            ...
            yield f"data: {json.dumps({'type': 'complete', 'response': partial, 'sources': [], 'timing': {'total_ms': elapsed_ms}})}\n\n"
    ...
    yield f"data: {json.dumps({'type': 'complete', 'response': response, 'sources': [], 'timing': {'total_ms': elapsed_ms}})}\n\n"
...
async def stream_vocabulary_response(question: str,
                                     ...
                                     conversation_id: str | None = None,
                                     message_history: list | None = None,
                                     running_summary: str | None = None) -> AsyncGenerator[str, None]:
...
async def stream_repo_archimate_response(
    ...
    if not phase2_has_artifact:
        yield f"data: {json.dumps({'type': 'error', 'content': 'ArchiMate model generation failed. The architecture notes were saved but the XML conversion encountered errors.'})}\n\n"
        yield f"data: {json.dumps({'type': 'complete', 'response': 'Repository analysis completed but ArchiMate model generation failed. You can retry or check the architecture notes artifact.'})}\n\n"
        return

    # Phase transition indicator
    yield f"data: {json.dumps({'type': 'status', 'agent': AGENT_LABELS['repo_analysis_agent'], 'content': 'ArchiMate model generated. Building architecture explorer...'})}\n\n"
    ...
    if html_content:
        ...
        yield f"data: {json.dumps({'type': 'artifact', 'artifact_id': artifact_id, 'filename': filename, 'content_type': 'text/html', 'summary': summary})}\n\n"
        yield f"data: {json.dumps({'type': 'complete', 'response': 'The interactive explorer has been saved as an artifact. You can download and open the HTML file in any browser.'})}\n\n"
    else:
        yield f"data: {json.dumps({'type': 'error', 'content': 'Failed to generate HTML explorer from architecture notes.'})}\n\n"
        yield f"data: {json.dumps({'type': 'complete', 'response': 'HTML explorer generation failed. The ArchiMate model was saved successfully.'})}\n\n"
        ...
        try:
            ...
            if execution_model in ("tree", "archimate", "principle") and loaded_artifact:
                if artifact_ct and artifact_ct.startswith("document/"):
                    ...
                    yield f"data: {json.dumps({'type': 'status', 'content': 'Loading document into context...'})}\n\n"
                else:
            ...
            if execution_model == ExecutionModel.GENERATION:
                ...
                yield f"data: {json.dumps({'type': 'status', 'content': _doc_status})}\n\n"
...
async def upload_file(request: Request):
    ...
    max_bytes = get_thresholds_value("get_upload_config", {"max_file_bytes": 10_485_760, "max_text_chars": 500_000, "upload_doc_max_chars": 200_000}).get(
    ...
    max_chars = get_thresholds_value("get_upload_config", {"max_file_bytes": 10_485_760, "max_text_chars": 500_000, "upload_doc_max_chars": 200_000}).get(
...
async def set_llm_settings(llm_settings: LLMSettings):
    ...
    # changes from the UI are silently ignored.
    _rebuild_agents()
