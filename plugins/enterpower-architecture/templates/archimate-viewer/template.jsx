...
/* ═══════════════════════════════════════════════════════════════
   ...
   PURPOSE
   Fixed, data-driven React component that renders an interactive
   ArchiMate 3.1 diagram in the browser. All rendering logic —
   element symbols, relation styles, layer colors, zoom/pan,
   highlight/dim, side panel — is baked in here and must not be
   modified. Only the INITIAL_VIEWS data block is replaced per
   model (see VIEWS INJECTION below).

   LOCATION — templates/archimate-viewer/
   This directory is a self-contained Vite project. The MCP server
   (mcp/visual-preview/src/index.ts) copies it to
   ~/.cache/archimate-preview/project/ and serves it at
   http://localhost:5173. The original here is the authoritative
   reference; the copy in the cache is the live working version.

   RELATIONS
   - Populated by: skills/archimate-visual-composer/SKILL.md
     (the assistant generates only the INITIAL_VIEWS data block)
   - Served by:    mcp/visual-preview/src/index.ts
                   (preview_start / preview_update tools)
   - Registered:   .mcp.json → "preview" server
   - Entry point:  templates/archimate-viewer/main.jsx

   VIEWS INJECTION
   The MCP server locates the block starting with
   "const INITIAL_VIEWS = {" and ending at the first unindented
   "};" and replaces it with the assistant's generated VIEWS data.
   All inner object closings use "}," so the regex is unambiguous.

   RENDERING (all fixed — do not modify):
   - Bean-webshop accuracy: ArchiMate 3.1 symbols, relation styles
   - NS-viewer UX: view-switcher, highlight/dim, side panel
   - Additions: header bar, draggable elements, legend as filter,
     SVG export placeholder (Phase 1)
   ═══════════════════════════════════════════════════════════════ */
