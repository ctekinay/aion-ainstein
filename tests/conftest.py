...
# ---------------------------------------------------------------------------
# MultiPluginRegistry singleton reset between tests
# ---------------------------------------------------------------------------

@pytest.fixture(autouse=True)
def _reset_multi_registry_singleton():
    """Reset the process-wide MultiPluginRegistry between tests.

    Tests that construct a registry directly (``MultiPluginRegistry()``)
    don't touch the singleton, but tests that call ``get_skill_registry()``
    or ``get_multi_registry()`` would otherwise share state via the
    process-global ``_global_multi``. Reset before each test so plugin
    discovery and on_reload state stay isolated.
    """
    from aion.skills.multi_registry import _reset_multi_registry_for_tests
