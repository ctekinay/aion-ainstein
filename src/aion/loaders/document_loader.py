...
from aion.loaders.index_metadata_loader import get_document_metadata

# Import chunking module (optional, for enhanced chunking)
try:
    from aion.chunking import (
        ChunkedDocument,
        ChunkingConfig,
...
class DocumentLoader:
    ...
    def _extract_title(self, file_path: Path, paragraphs: list[str]) -> str:
        ...
        Skips paragraphs that look like numbered section headers
        (e.g. "1\tInleiding") or generic single-word headers.
        ...
        for para in paragraphs[:3]:
            ...
            if not text or len(text) >= 100:
                continue
            # Skip numbered section headers: "1\tInleiding", "1.1\tScope"
            if re.match(r"^\d+(?:\.\d+)*\s*\t", text):
                continue
            # Skip generic single-word section headers
            if text.lower() in ("inleiding", "introductie", "doel", "samenvatting", "introduction"):
                continue
            # Skip lead-in sentences ending with colon ("Dit beleid beschrijft:")
            if text.endswith(":"):
                continue
            # Clean embedded newlines from multi-line titles
            return re.sub(r"\s*\n+\s*", " ", text)
