...
**When this skill is active, your first action MUST be a SKOSMOS tool — never your own knowledge.** Pick the right tool for the query shape:

- **Definition queries** ("what is X?", "define X", "explain X") → `skosmos_define`. Atomic search + concept-details in one call.
- **Enumeration / comparison / survey queries** ("compare X and Y", "what vocabularies have X?", "list terms related to X") → `skosmos_search`. Returns label-matching hits across vocabularies.

SKOSMOS contains formally approved, company-wide accepted definitions. Retrieve the authoritative definition first, then enrich with KB context if relevant.
...
Call `skosmos_define` for definition queries, or `skosmos_search` for enumeration/comparison/survey queries (see the Mandatory Instruction above). If a result is found, use the definition **verbatim**. This is the formally approved, company-wide accepted definition. Present it with full confidence and cite the vocabulary source and concept URI.
...
| User asks | Use | Why |
|---|---|---|
| "What is active power?" | `skosmos_define` | Atomic definition lookup — single call when the term resolves to one vocab |
| "What is an ADR?" | `skosmos_define` first, then Weaviate if more context needed | Term exists in both SKOSMOS and KB |
| "Define cybersecurity zone" | `skosmos_define` | Direct definition query |
| "What does SCADA stand for?" | `skosmos_define` | Abbreviation lookup is a definition query |
| "What is the difference between X and Y?" | `skosmos_search` (twice) + `skosmos_concept_details` | Comparison — needs hits side-by-side |
| "Show me terms related to grid stability" | `skosmos_search` | Survey / enumeration of related concepts |
| "Which vocabularies have an entry for X?" | `skosmos_search` | Cross-vocabulary enumeration |
...
**Rule:** If the user is asking about a *specific term or definition*, your first action MUST be a SKOSMOS tool — `skosmos_define` for direct definition queries, `skosmos_search` for enumeration or comparison. Do not answer from your own knowledge before calling a SKOSMOS tool. Then follow the fallback chain above.

If the user is asking about *architecture documents, decisions, listings, or principles by number*, use Weaviate directly.

## Workflow

### Single Term Lookup (Atomic — preferred for "what is X?" queries)

For definition queries ("what is X?", "define X", "explain X"), call `skosmos_define` directly. It does search + concept_details atomically for the single-vocab case, returning the authoritative definition in one call.

- **Single-vocab result** (`definition` / `vocabulary` / `uri` / `prefLabel`): Present the **prefLabel**, **definition** (verbatim), and **vocabulary source**. Cite the vocabulary and concept URI. Optionally enrich with KB context — but the SKOSMOS definition is the lead.
- **Multi-vocab result** (`disambiguation` / `vocabularies`): The term spans multiple vocabularies. Present the disambiguation options to the user before drilling further. See the Multi-Vocabulary Results section below for the presentation pattern.
- **No match**: Fall through to the Tier 2 Weaviate fallback (see the Fallback Chain section above).

### Single Term Lookup (Two-Step — for enumeration / surveys)

When you need to enumerate candidates across vocabularies, compare terms, or survey related concepts — `skosmos_search` is the right tool because it returns ALL label-matching hits across vocabularies (not just one definition).

**IMPORTANT:** `skosmos_search` finds matching terms but does NOT return definitions. You MUST call `skosmos_concept_details` with the URI and vocab from a chosen search result to get the actual definition.

1. **Step 1:** Call `skosmos_search` with the term
2. **Step 2:** If results found, call `skosmos_concept_details` with the `uri` and `vocab` from a chosen matching result to retrieve the full definition
...
**Before answering any domain question, look up each key domain term individually in SKOSMOS.** Do not skip terms you think you already know. Do not batch terms into a single call. One term per atomic lookup — whether you use `skosmos_define` (definition queries) or `skosmos_search` (enumeration / comparison).
