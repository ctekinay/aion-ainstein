...
Contains the 8 Weaviate search tools (search_architecture_decisions,
search_principles, search_policies, list_all_adrs, list_all_principles,
list_all_policies, search_by_team, get_collection_stats) plus all
...
class RAGToolkit:
    ...
    def search_architecture_decisions(
        ...
        if len(adr_numbers) >= 2:
            ...
            return sorted(
                seen.values(), key=lambda x: x.get("file_path", "")
            )
        ...
        if len(adr_numbers) == 1:
            ...
            if is_dar_query:
                ...
                adr_filter = (
                    ...
                    & Filter.by_property("title").not_equal(
    ...
    def search_principles(
        ...
        if len(pcp_numbers) >= 2:
            ...
            for obj in results.objects:
                ...
                if not pn or pn in seen:
                    continue
                if not is_dar_query and title.startswith(
                    "Principle Approval Record"
                ):
                    continue
        ...
        if len(pcp_numbers) == 1:
            ...
            if is_dar_query:
                pcp_filter = Filter.by_property("principle_number").equal(
                    padded
                )
            else:
                pcp_filter = (
