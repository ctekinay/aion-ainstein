...
def _build_rag_agent(toolkit: RAGToolkit) -> Agent[SessionContext, str]:
    ...
    @agent.tool
    def list_adrs(ctx_: RunContext[SessionContext]) -> list[dict]:
    ...
    @agent.tool
    def list_policies(ctx_: RunContext[SessionContext]) -> list[dict]:
    ...
    @agent.tool
    def list_dars(ctx_: RunContext[SessionContext]) -> list[dict]:
        """List all Decision Approval Records (DARs) from the knowledge base.

        ALWAYS use this tool when the user asks about approval records,
        who approved a decision/principle, or wants to see DARs.
        Examples: "Show all approval records", "List all DARs",
        "What DARs exist?", "Who approved what?"

        DARs are stored alongside ADRs and PCPs in Weaviate. Each result
        includes a dar_source field ("ADR" or "PCP") indicating which
        collection the approval record came from.
        """
        if ctx_.deps.check_iteration_limit():
            return [{"error": "Tool call limit reached"}]
        elapsed = elapsed_ms(ctx_.deps._query_start)
        ...
        ctx_.deps.emit_event({
            "type": "status",
            "content": f"Found {len(result)} DARs",
            "elapsed_ms": elapsed_ms(ctx_.deps._query_start),
        })
        return result
...
- For technical terms, provide clear explanations"""
    ...
    async def _generate_with_openai(
        ...
        if is_reasoning_model(model):
            completion_kwargs["max_completion_tokens"] = token_limits.get("rag_reasoning", 1000)
        else:
            completion_kwargs["max_tokens"] = token_limits.get("rag_standard", 1000)
