...
def _load_principle_owner_map() -> dict[str, str]:
    ...
    """
    # registry-index.md is a reference file owned by the esa-document-ontology
    # skill. Resolve it through the OWNING plugin's skills_dir, never a
    # hardcoded path: esa-document-ontology lives in the esa-workflow plugin
    # (it was moved kernel→esa-workflow as ESA-specific; owner-routing is
    # exactly why this keeps working across that move).
    from aion.skills.registry import get_skill_registry

    loader = get_skill_registry().get_loader_for_skill("esa-document-ontology")
    registry_path = (
        loader.skills_dir
    ...
    if registry_path is None or not registry_path.exists():
        logger.warning(
            "Ownership registry not found (esa-document-ontology skill "
            "unavailable or registry-index.md missing); principle ownership "
            "will not be corrected during ingestion"
        )
        return {}
