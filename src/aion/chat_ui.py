...
from aion.memory.summarizer import SUMMARIZE_TRIGGER_COUNT, generate_rolling_summary
...
from aion.tools.rag_search import _get_truncation
...
async def lifespan(app: FastAPI):
    ...
    global _weaviate_client, _rag_agent, _vocabulary_agent, _archimate_agent, _principle_agent, _generation_pipeline, _persona
...
def _extract_sources(objects: list, simple: bool = False) -> list[dict]:
    ...
    """
    display_limit = _get_truncation().get("source_display_limit", 25)
    ...
    async def event_generator():
        ...
        try:
            ...
            if persona_result.direct_response is not None:
                ...
                pixel_registry.thinking("persona", f"Direct response ({persona_result.intent})")
            ...
            def _capture_event(event):
                ...
                try:
                    ...
                    if evt_type == "complete":
                        ...
                        # Route pixel agent animations using the structured agent field
                        if agent == AGENT_LABELS["persona"]:
                            pixel_registry.tool_call("persona", "classify_intent", content)
                        elif agent == AGENT_LABELS["orchestrator"]:
                            pixel_registry.tool_call("orchestrator", "orchestrate", content)
                            pixel_registry.tool_call(current_pixel_agent, "orchestrator_step", content)
                        elif agent == AGENT_LABELS["synthesis"]:
                            pixel_registry.tool_call(current_pixel_agent, "synthesis", content)
                        elif content.startswith("Found "):
                            pixel_registry.tool_result(current_pixel_agent, content)
                        else:
                            pixel_registry.tool_call(current_pixel_agent, "processing", content)
                    elif evt_type == "decision":
                        content = data.get("content", "")
                        agent = data.get("agent", "")
                        thinking_steps.append({"text": content, "type": evt_type, "agent": agent})
                        # decision content is now human-readable (rewritten by emit_event)
                        # extract tool name from the "tool" field if present, else use content
                        tool_name = data.get("tool", content.split(" ")[0] if content else "unknown")
                        pixel_registry.tool_call(current_pixel_agent, tool_name, content)
                        ...
                        pixel_registry.speech("persona", "Got it!", 2.0)
                        pixel_registry.idle("persona")
                        if execution_model:
                            pixel_registry.speech(current_pixel_agent, "Working on it!", 2.5)
                            pixel_registry.tool_call(current_pixel_agent, "handle_query", f"Handling {data.get('intent', 'query')}...")
                        ...
                        pixel_registry.tool_call("archimate_agent", "generate_archimate",
