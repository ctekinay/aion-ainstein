...
class OllamaEmbeddings:
    ...
    def embed_batch(self, texts: list[str]) -> list[list[float]]:
            ...
            try:
                ...
                if attempt < settings.embedding_max_retries - 1:
                    logger.warning(
                        ...
                        f"Retrying in {settings.embedding_retry_delay}s..."
                    )
                    time.sleep(settings.embedding_retry_delay)
                continue
            except Exception:
                break
        ...
        logger.warning(
            f"Batch embedding failed after {settings.embedding_max_retries} attempts. "
            ...
            for attempt in range(settings.embedding_max_retries):
                try:
                    ...
                    if attempt < settings.embedding_max_retries - 1:
                        ...
                        logger.error(
                            f"Failed to embed text {i} after {settings.embedding_max_retries} attempts: {e}. "
    ...
    def client(self):
        if self._client is None:
            ...
            self._client = OpenAI(
                ...
                timeout=httpx.Timeout(settings.timeout_long_running, connect=10.0),
            )
        return self._client
        ...
        for attempt in range(settings.embedding_max_retries):
            try:
                ...
                if attempt == settings.embedding_max_retries - 1:
                    raise
                logger.warning(
                    f"Embedding retry {attempt + 1}/{settings.embedding_max_retries}: {e}"
                )
                ...
                for attempt in range(settings.embedding_max_retries):
                    try:
                        ...
                        if attempt == settings.embedding_max_retries - 1:
                            raise
                        logger.warning(
                            f"Batch embedding retry {attempt + 1}/{settings.embedding_max_retries}: {e}"
                        )
                        time.sleep(settings.embedding_retry_delay * (attempt + 1))
