...
- Avoid curly-brace placeholders like `{n}`, `{number}`, `{name}` in headings or prose — they look like unresolved template variables. Use descriptive text instead: "ADR approval records" not "ADR approval records (ADR.{n}D)".
