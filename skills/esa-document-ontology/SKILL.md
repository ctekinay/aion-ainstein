...
- "What ADRs exist?" → Use Weaviate `list_all_adrs` (listing question)
- "Who approved ADR.12?" → Use this skill + Weaviate (approval lookup)

When a user asks "What is [term]?", always call `skosmos_search` first to get the formal definition. Then use this skill to add structural context if relevant.
