...
Bypasses the RAG agent for tasks that need content generation
...
# Extension + fallback-prefix mapping for artifact filenames, keyed by content_type.
# Archimate (.archimate.xml) is handled separately — compound extension not derivable from MIME.
_EXT_MAP = {
    "text/html": (".html", "explorer"),
    "text/plain": (".txt", "model"),
    "text/csv": (".csv", "export"),
}
...
class GenerationPipeline:
    ...
    # Fields excluded from full-content fetches (same as RAGToolkit)
    ...
    async def generate(
        ...
        Args:
            ...
            source_text: Pre-computed source content (e.g., from RepoAnalysisAgent).
                When provided, all Weaviate/GitHub fetch logic is bypassed.
        ...
        source_metadata = build_source_metadata(sources)
        ...
        loader = get_skill_registry().get_loader_for_skill(skill_entry.name)
        skill = loader.load_skill(skill_entry.name) if loader else None
        system_prompt = skill.get_injectable_content() if skill else ""

        # Integration Point A: inject registry context for YAML-pipeline skills
        # (gated on yaml_pipeline rather than a hardcoded skill name — ISS-002).
        registry_context = ""
        if skill_entry.yaml_pipeline:
        ...
        if is_refinement and yaml_refinement:
            ...
            if skill_entry.content_type == "text/html":
                gen_instruction = (
                    "Generate the complete, self-contained HTML file following "
                    "the instructions and template in your system prompt. "
                    "Output ONLY the HTML (starting with <!DOCTYPE html> or <html>). "
                    "Do not include commentary, markdown fences, or any other text."
                )
            elif "archimate" in skill_entry.name:
                gen_instruction = (
                    "Generate the YAML following the schema in your instructions. "
                    "Do not include commentary or any other text."
                )
            else:
                gen_instruction = (
                    "Generate the output following the instructions in your system prompt. "
                    "Do not include commentary or any other text."
                )
            user_prompt = (
                f"SOURCE CONTENT:\n{source_text}\n\n"
        ...
        elif skill_entry.content_type == "text/html":
            max_tokens_override = 32768  # HTML explorers are large self-contained files
        ...
        # Strip markdown fences from HTML output (LLMs sometimes wrap in ```html fences)
        if skill_entry.content_type == "text/html":
            raw_output = re.sub(r"^```(?:html)?\s*\n?", "", raw_output.strip())
            raw_output = re.sub(r"\n?```\s*$", "", raw_output).strip()

        # Step 3b: YAML → XML conversion (skills opt in via yaml_pipeline — ISS-002)
        yaml_source = None
        change_summary = None
        pipeline_info = {
            "pipeline": "xml",
        ...
        if skill_entry.yaml_pipeline:
            ...
            if yaml_text:
                ...
                # Backfill source_metadata from generated YAML if empty
                # (e.g., repo-analysis flow where source_text is pre-provided
                # and the Weaviate fetch was skipped).
                source_metadata = self._backfill_source_metadata(yaml_text, source_metadata)

                # Enrich with dct properties before XML conversion.
                # Always call — strips source_ref even when no metadata matches.
                yaml_text = enrich_yaml_with_dct(yaml_text, source_metadata)
                ...
                try:
                    ...
                    if yaml_text:
                        ...
                        source_metadata = self._backfill_source_metadata(
                            yaml_text, source_metadata
                        )
                        yaml_text = enrich_yaml_with_dct(
                            yaml_text, source_metadata
                        )
                        convert_start = time.perf_counter()
                        try:
                            ...
                            logger.warning(
                                "[generation] YAML conversion failed after retry: %s\n"
                                "[generation] Retry YAML (first 500 chars): %s",
                                e2, yaml_text[:500] if yaml_text else "(empty)",
                            )
    ...
    def _fetch_documents(
        ...
        """Fetch documents by reference, aggregating all chunks per document.

        Args:
            refs: Document references like ["ADR.29", "PCP.10"].
            collection_name: Weaviate collection name.
            id_prop: Property that holds the document identifier (e.g. "adr_number").
            exclude_fn: Optional callable(properties_dict) -> bool. Returns True
                to exclude the object. Applied after fetching, not as a Weaviate
                filter (avoids the range + not_equal bug).
        """
        ...
        if len(numbers) >= 2:
            ...
            doc_filter = (
                Filter.by_property(id_prop).greater_or_equal(start)
                & Filter.by_property(id_prop).less_or_equal(end)
            )
        else:
            padded = str(numbers[0]).zfill(4)
            doc_filter = Filter.by_property(id_prop).equal(padded)

        results = collection.query.fetch_objects(
            filters=doc_filter, limit=500, return_properties=props,
        )

        # Aggregate all chunks per document (generation needs full content).
        docs: dict[str, dict] = {}
        for obj in results.objects:
            doc_id = obj.properties.get(id_prop, "")
            if not doc_id:
                continue
            if exclude_fn and exclude_fn(obj.properties):
                continue

            if doc_id not in docs:
                docs[doc_id] = {k: obj.properties.get(k, "") for k in props}
                docs[doc_id]["kb_uuid"] = str(obj.uuid)
            else:
                chunk_content = obj.properties.get("content", "")
                if chunk_content:
                    docs[doc_id]["content"] = (
                        docs[doc_id].get("content", "") + "\n\n" + chunk_content
                    )

        return sorted(docs.values(), key=lambda x: x.get(id_prop, ""))
    ...
    def _fetch_adrs(self, adr_refs: list[str]) -> list[dict]:
        ...
        def _exclude_non_dar(props: dict) -> bool:
            if is_dar:
                return False
            ...
            return title.startswith("Decision Approval Record")

        return self._fetch_documents(
            adr_refs, "ArchitecturalDecision", "adr_number", _exclude_non_dar,
        )

    def _fetch_pcps(self, pcp_refs: list[str]) -> list[dict]:
        """Fetch Principle documents by reference, returning full content."""
        def _exclude_par(props: dict) -> bool:
            return props.get("title", "").startswith("Principle Approval Record")

        return self._fetch_documents(
            pcp_refs, "Principle", "principle_number", _exclude_par,
        )

    def _backfill_source_metadata(
        self, yaml_text: str, source_metadata: dict,
    ) -> dict:
        """Extract source_ref values from generated YAML and fetch KB metadata.

        When source_text is pre-provided (e.g., repo-analysis flow), the normal
        Weaviate fetch is skipped, leaving source_metadata empty. This method
        scans the LLM-generated YAML for source_ref fields, fetches the
        referenced documents from Weaviate, and builds source_metadata so that
        enrich_yaml_with_dct() can resolve DCT properties.

        Returns the original source_metadata if it's already populated, or
        a new dict built from the fetched documents.
        """
        if source_metadata:
            return source_metadata  # already have metadata, nothing to do
        ...
        # Collect unique source_ref values from generated elements
        doc_refs = set()
        ...
        # Fetch from Weaviate and build metadata
        logger.info("[generation] backfilling source_metadata for %d doc refs from YAML", len(doc_refs))
        sources = self._fetch_by_doc_refs(sorted(doc_refs))
        if sources:
            return build_source_metadata(sources)
    ...
    def _get_registry_prompt_limit() -> int:
        """Read element_registry.prompt_limit from runtime.yaml (system infra)."""
        try:
            from aion.config.runtime import get_runtime_value
            return get_runtime_value("element_registry.prompt_limit", 30)
        except Exception:
            logger.warning("Failed to read registry prompt_limit, using default 30")
            return 30
    ...
    def _reconcile_with_registry(
        ...
        try:
            ...
            logger.warning("YAML parse error in reconciliation, skipping: %s", e)
            return yaml_text
        if not data or "elements" not in data:
            return yaml_text
    ...
    async def _fetch_github_content(
        ...
        max_readme = max(settings.min_readme_chars_per_ref, settings.max_readme_chars // len(github_refs))
    ...
    async def _call_openai(
        ...
        client = OpenAI(**settings.get_openai_client_kwargs(
            settings.effective_rag_provider, timeout=settings.timeout_generation,
        ))
        model = settings.effective_rag_model
        ...
        # Stream to keep the connection alive during reasoning model
        # thinking phase — prevents intermediate network timeouts on
        # idle connections (observed: ~60s drops despite 600s client timeout).
        start = time.perf_counter()
        ...
        if usage_data:
            token_stats["prompt_tokens"] = usage_data.prompt_tokens or 0
            token_stats["completion_tokens"] = usage_data.completion_tokens or 0
            logger.info(
    ...
    async def _call_ollama(
        ...
        start = time.perf_counter()
        async with httpx.AsyncClient(timeout=settings.timeout_generation) as client:
    ...
    def _save_artifact(
        ...
        """
        from aion.chat_ui import save_artifact
        ...
        if skill_entry.content_type == "text/html" and "<html" not in content.lower():
            logger.warning("[generation] Skipping artifact save — no HTML in content")
            return None

        filename = self._derive_filename(content, doc_refs, skill_entry)
        if skill_entry.content_type:
            content_type = skill_entry.content_type
        elif "archimate" in skill_entry.name:
    ...
    def _derive_filename(
        ...
        if "archimate" in skill_entry.name:
            ext, fallback_prefix = ".archimate.xml", "model"
        else:
            ext, fallback_prefix = _EXT_MAP.get(
                getattr(skill_entry, "content_type", ""), (".txt", "model")
            )
        ...
        # 3. Fallback
        return f"{fallback_prefix}_{ts}{ext}"
    ...
    def _build_response(
        ...
        # Artifact content (XML/HTML) goes in the download card, not inline.
        # Only show non-artifact content inline (e.g., if generation failed).
        _content_type = getattr(skill_entry, "content_type", "") if skill_entry else ""
        _is_html_skill = _content_type == "text/html"
        _is_artifact = (
            "<model" in content
            or "<?xml" in content
            or (_is_html_skill and "<html" in content.lower())
        )
        if not _is_artifact:
    ...
    def _try_diff_merge(
        ...
        """
        from aion.tools.yaml_to_xml import apply_yaml_diff
...
# ---------------------------------------------------------------------------
# Module-level helpers (no Weaviate client required)
# ---------------------------------------------------------------------------

_synthesis_logger = structlog.get_logger(__name__ + ".synthesis")


async def _iter_ollama_tokens(resp) -> AsyncGenerator[str, None]:
    """Yield clean text tokens from a streaming Ollama NDJSON response.

    Suppresses <think>...</think> content across chunk boundaries using a
    stateful buffer — handles partial tags that span multiple NDJSON lines.
    """
    in_think = False
    buf = ""
    async for line in resp.aiter_lines():
        ...
        try:
            ...
            logger.warning("Skipping malformed NDJSON line from Ollama: %s", line[:200])
            continue
        ...
        while True:
            if not in_think:
                pos = buf.find("<think>")
                if pos == -1:
                    # Hold back enough chars to detect an incoming partial tag
                    safe = buf[: len(buf) - len("<think>")]
                    if safe:
                        yield safe
                        buf = buf[len(safe):]
                    break
                yield buf[:pos]
                buf = buf[pos + len("<think>"):]
                in_think = True
            else:
                pos = buf.find("</think>")
                if pos == -1:
                    # Discard think content; hold last 8 chars for partial-tag detection
                    buf = buf[max(0, len(buf) - len("</think>")):]
                    break  # wait for more data; re-scans last 8 bytes on next iteration
                buf = buf[pos + len("</think>"):]
                in_think = False

        if done:
            # Edge case: stream ends while in_think (model opened <think> but never closed).
            # Correct: suppress the unclosed think content entirely.
            if buf and not in_think:
                yield strip_think_tags(buf)  # flush residual, catch partial artifacts
            return
...
async def stream_synthesis_response(
    ...
    """Yield synthesis LLM response tokens as they arrive.

    Called after RAG completes when Persona sets complexity=="multi-step" and
    the user's message contains pasted content. Combines the original message
    (paste intact) with KB results to produce a grounded final response.

    When artifact_context is provided (cross-reference mode), the uploaded
    document is included so the synthesis LLM can compare KB results against it.

    Yields raw text tokens -- chat_ui.py wraps each into SSE text events.
    synthesis_instruction defaults to a generic combine instruction if None.
    """
    instruction = synthesis_instruction or (
        "Combine the user's input with the knowledge base results "
        "to provide a comprehensive response."
    )
    # NOTE: Keep in sync with skills/rag-quality-assurance/SKILL.md abstention rules.
    # If this grows beyond basic rules, inject via skill registry instead.
    _quality_rules = (
        "Rules:\n"
        "- Cite sources as ADR.XX or PCP.XX for every factual claim.\n"
        "- Match response depth to the question's complexity.\n"
        "- The knowledge base results above ARE the complete retrieval. Work with what is provided.\n"
        "- NEVER claim a document is 'not retrievable' if its content appears in the results above.\n"
        "- If a result contains only metadata (title, status, file path) without content sections,\n"
        "  note this explicitly rather than claiming the document doesn't exist.\n"
        "- If the retrieved results genuinely don't answer the question, say so briefly."
    )
    # artifact_context goes in the user message, not the system prompt.
    # Models process user message content reliably; long system prompts
    # get truncated or deprioritized by smaller models.
    system_prompt = (
        f"{instruction}\n\nKnowledge base results:\n\n{rag_response}\n\n"
        f"{_quality_rules}"
    )
    _user_message = original_message
    if artifact_context:
        _user_message = f"{artifact_context}\n\n## QUESTION:\n{original_message}"

    provider = settings.effective_rag_provider
    model = settings.effective_rag_model

    _synthesis_logger.info(
        "synthesis_start",
        provider=provider,
        model=model,
        instruction_set=synthesis_instruction is not None,
    )
    ...
    if provider in ("github_models", "openai"):
        ...
        async with AsyncOpenAI(**settings.get_openai_client_kwargs(
            provider, timeout=settings.timeout_generation,
        )) as client:
            # Use cross-reference budget when document is in context
            from aion.config.runtime import get_runtime_value
            _token_limits = get_runtime_value("llm_token_limits", {})
            _default_budget = 8192
            if artifact_context:
                _default_budget = _token_limits.get("cross_reference_synthesis", 16384)
            token_kwarg = (
                {"max_completion_tokens": _default_budget}
                if is_reasoning_model(model)
                else {"max_tokens": _default_budget}
            )
        ...
        async with httpx.AsyncClient(timeout=settings.timeout_long_running) as http_client:
            async with http_client.stream(
                ...
                async for token in _iter_ollama_tokens(resp):
                    text_so_far.append(token)
                    yield token
    ...
    _synthesis_logger.info(
        "synthesis_complete",
        response_chars=sum(len(t) for t in text_so_far),
