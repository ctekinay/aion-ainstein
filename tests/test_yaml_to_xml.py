...
from aion.tools.yaml_to_xml import (
    _prop_def_id,
...
relationships:
  - type: Serving
    ...
    target: nonexistent
"""
        # Invalid relationships are dropped with a warning, not raised
        result = yaml_to_archimate_xml(yaml_str)
        xml = result[0] if isinstance(result, tuple) else result
        assert "nonexistent" not in xml  # relationship was dropped
        assert "ApplicationComponent" in xml  # element still present
...
"""
        import logging
        with caplog.at_level(logging.WARNING, logger="aion.tools.yaml_to_xml"):
            xml_str, info = yaml_to_archimate_xml(yaml_str)
...
class TestViewpoints:
    ...
    def test_viewpoints_constant_covers_all_layers(self):
        ...
        from aion.tools.archimate import VALID_ELEMENT_TYPES
        from aion.tools.yaml_to_xml import VIEWPOINTS
    ...
    def test_generate_viewpoint_xml_application(self):
        ...
        from aion.tools.yaml_to_xml import generate_viewpoint_xml
    ...
    def test_generate_viewpoint_merge_roundtrip(self):
        ...
        from aion.tools.archimate import merge_archimate_view, validate_archimate
        from aion.tools.yaml_to_xml import generate_viewpoint_xml
