...
class _PythonExtractor(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node):
        ...
        self.classes.append({
            ...
            "_name_refs": {n.id for n in ast.walk(node) if isinstance(n, ast.Name)},
        })
            ...
            if internal_names and file_info.get("classes"):
                for cls in file_info["classes"]:
                    if isinstance(cls, dict):
                        ...
                        # Consume _name_refs: scope collaborators to names actually
                        # referenced in the class body, not all file-level imports.
                        bases = set(cls.get("bases", []))
                        name_refs = cls.pop("_name_refs", set())
                        collaborators = [
                            n for n in internal_names
                            if n not in bases
                            and n != cls.get("name")
                            and n in name_refs
                        ]
