...
def _get_skill_config(getter_name: str, default):
    """Read a plugin-tuning value via the registry, gated on rag-quality-assurance.

    Falls back to the provided default if the skill registry fails to load,
    the rag-quality-assurance skill is disabled, or the loader raises.
    """
    if not _SKILLS_AVAILABLE:
        return default
    try:
        return get_skill_registry().get_skill_tuning(
            "rag-quality-assurance", getter_name, default,
        )
    except Exception:
...
class RAGToolkit:
    ...
    def _build_result(
        ...
        for key in props:
            ...
            # Strip 4-digit internal padding to canonical 2-digit display format
            # (ADR.00, PCP.10 — matching ESA registry convention)
            if key in ("adr_number", "principle_number") and isinstance(val, str) and val:
    ...
    def list_adrs(self) -> list[dict]:
        ...
        # list_* uses higher truncation than search_* — evaluation scenarios
        # need full text (Statement + Rationale + Implications).
        content_limit = _get_truncation().get("list_content_max_chars", 4000)
        seen = {}
        for obj in results.objects:
