...
class MarkdownLoader:
    ...
    def _is_content_file(path: Path) -> bool:
        """Return False for non-content files (index pages, templates)."""
        name = path.name.lower()
        return name != "index.md" and "template" not in name
    ...
    def load_adrs(self, adr_path: Path) -> Iterator[dict]:
        ...
        """
        adr_files = sorted(f for f in adr_path.glob("*.md") if self._is_content_file(f))
    ...
    def load_principles(self, principles_path: Path) -> Iterator[dict]:
        ...
        """
        principle_files = sorted(f for f in principles_path.glob("*.md") if self._is_content_file(f))
